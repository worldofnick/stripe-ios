...
class PaymentSheetStandardLPMUIOneTests: PaymentSheetStandardLPMUICase {
    ...
    func testRefreshEndpointUsingCashAppPay() throws {
        ...
        // Close the webview once its browser controls are actually ready, to simulate cancel.
        let closeButton = app.otherElements["TopBrowserBar"].buttons["Close"]
        XCTAssertTrue(
            closeButton.waitForExistence(timeout: 30.0),
            "Cash App Pay redirect webview did not present in time"
        )
        closeButton.forceTapWhenHittableInTestCase(self)
