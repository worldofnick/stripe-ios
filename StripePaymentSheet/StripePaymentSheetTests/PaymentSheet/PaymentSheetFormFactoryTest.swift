...
class PaymentSheetFormFactoryTest: XCTestCase {
    ...
    func testUnsupportedPaymentMethodForm() {
        // Given an unsupported payment method
        ...
        // When building its form
        STPAssertTestUtil.shouldSuppressNextSTPAlert = true
        _ = factory.make()

        // Then the unsupported state fails loudly and is logged
        XCTAssertTrue(STPAssertTestUtil.lastAssertMessage.contains("unsupportedPaymentMethod"))
        let errorAnalytic = analyticsClient._testLogHistory.first!
        XCTAssertEqual(
            errorAnalytic["event"] as? String,
            STPAnalyticEvent.unexpectedPaymentSheetFormFactoryError.rawValue
        )
        XCTAssertEqual(errorAnalytic["payment_method"] as? String, "card_present")
        XCTAssertEqual(errorAnalytic["error_code"] as? String, "unsupportedPaymentMethod")
    }
