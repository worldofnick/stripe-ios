...
final class EmbeddedPaymentElementRevertSelectionTests: XCTestCase {
    ...
    func testManageScreenDeleteSelectedPM_fallsBackToRemaining() throws {
        // Given the saved card is selected
        let customerID = "cus_epe_delete_selected"
        defer { CustomerPaymentOption.setDefaultPaymentMethod(nil, forCustomer: customerID) }
        ...
        let sut = makeEmbeddedPaymentElement(configuration: config, loadResult: loadResult)
        sut.embeddedPaymentMethodsView.didTap(rowButton: sut.embeddedPaymentMethodsView.getRowButton(accessibilityIdentifier: "•••• 4242"))
        XCTAssertEqual(sut.paymentOption?.label, "•••• 4242")

        // When the selected card is deleted in the manage screen
        ...
        sut.didComplete(viewController: manageVC, with: bank, latestPaymentMethods: [bank], didTapToDismiss: false, defaultPaymentMethod: nil)

        // Then the selection falls back to the remaining payment method
        XCTAssertEqual(sut.paymentOption?.label, "••••6789")
    }

    func testManageScreenDeleteOtherPM_selectionUntouched() throws {
        // Given the saved card is selected
        let customerID = "cus_epe_delete_other"
        defer { CustomerPaymentOption.setDefaultPaymentMethod(nil, forCustomer: customerID) }
        let cardA = STPPaymentMethod._testCard()
        let bank = STPPaymentMethod._testUSBankAccount()
        let config = makeConfiguration(customerID: customerID)
        ...
        // When a different saved PM is deleted in the manage screen
        let manageVC = VerticalSavedPaymentMethodsViewController(
            configuration: PaymentSheet.Configuration._testValue_MostPermissive(),
            intent: ._testValue(),
            selectedPaymentMethod: cardA,
        ...
        sut.didComplete(viewController: manageVC, with: cardA, latestPaymentMethods: [cardA], didTapToDismiss: false, defaultPaymentMethod: nil)

        // Then the selection is untouched
        XCTAssertEqual(sut.paymentOption?.label, "•••• 4242")
    }

    func testSameRowFormCancel_afterEdit_restoresCommittedCard() throws {
        ...
        // When the user re-opens the same row's form, edits it, then cancels
        sut.embeddedPaymentMethodsView.didTap(rowButton: sut.embeddedPaymentMethodsView.getRowButton(accessibilityIdentifier: "Card"))
        ...
        // Then the committed card is restored, not cleared, including the form contents
        XCTAssertEqual(sut.paymentOption?.label, "•••• 4242")
