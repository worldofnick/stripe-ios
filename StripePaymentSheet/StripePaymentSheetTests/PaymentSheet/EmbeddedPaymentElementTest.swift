...
class EmbeddedPaymentElementTest: XCTestCase {
    ...
    // MARK: - Checkout billing sync on selection commit

    func testCheckout_commitSavedSelection_informsDelegateAndSyncsBilling() async throws {
        ...
        sut.commitSelectedPaymentOption()
    ...
    func testCheckout_commitSelectionWithNothingToSync_informsDelegateImmediately() async throws {
        ...
        // When the selection is committed, there's nothing to sync...
        sut.commitSelectedPaymentOption()

        // ...so the delegate is informed synchronously and no sync is kicked off
        XCTAssertTrue(delegateDidUpdatePaymentOptionCalled)
        XCTAssertTrue(checkout.pendingOperations.isEmpty)
    }

    func testCheckout_commitSelection_syncFails_leavesSessionBillingUnchanged() async throws {
