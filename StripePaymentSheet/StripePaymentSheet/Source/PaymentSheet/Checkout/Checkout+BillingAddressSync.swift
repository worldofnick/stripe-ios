//
//  Checkout+BillingAddressSync.swift
//  StripePaymentSheet
//
...
extension Checkout {
    ...
    @MainActor
    static func syncBillingIfNeeded(
        intent: Intent,
        ...
        guard let (checkout, billingDetails) = intent.checkoutRequiringBillingSync(for: paymentOption) else {
            completion()
            return
        }
